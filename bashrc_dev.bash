# .bashrc for WSL using terminator
# Last update AH 10/17/2018
...
# --- Patch for using Windows shortcut to start terminator --- #
if [ -t 1 ]; then
  cd ~
fi

# --- Environmental variables --- #
export HISTSIZE=1000  # Change history size
export HISTFILESIZE=1000  # Change history size
...
alias git='git.exe'         # set default git to use Windows binaries for work compatibility
alias lgit='/usr/bin/git'   # alias to call Linux's git in case if needed to write to WSL
...
export sherlock='ahho@login.sherlock.stanford.edu'
export sherlockdtn='ahho@dtn.sherlock.stanford.edu'
export oak='ahho@oak-dtn.stanford.edu:/oak/stanford/groups/wjg/ahho'
export sshaws='ubuntu@18.215.102.0'
...
alias sshaws='ssh -i ~/.ssh/aws.pem $sshaws'
alias scpaws='scp -i ~/.ssh/aws.pem'
# - Stanford - #
